  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Report issues with a fundi or completed job</p>
      </div>
      ...
      <div className="stat-card space-y-4 animate-fade-in">
        ...
        <div className="space-y-2">
          <Label>Select Job & Fundi</Label>
          <Select value={selectedJobId} onValueChange={setSelectedJobId}>
            <SelectTrigger className="bg-muted/50"><SelectValue placeholder="Select a job..." /></SelectTrigger>
            <SelectContent>
              {jobs.map(j => (
        ...
        <div className="space-y-2">
          ...
          <Textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Describe the issue in detail..." className="bg-muted/50 min-h-[100px]" />
        </div>
      ...
      <div className="space-y-4 animate-fade-in" style={{ animationDelay: "100ms" }}>
        ...
        {complaints.length === 0 ? (
          <div className="stat-card text-center py-12 text-muted-foreground text-sm">No complaints submitted yet.</div>
        ) : (
          complaints.map(c => (
